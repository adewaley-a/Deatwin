...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    return () => { unsub(); if(socket.current) socket.current.disconnect(); };
  }, [roomId, role]);
    ...
    const render = () => {
        ...
        ref.current.forEach((b, i) => {
          ...
          for (let t of targs) {
              ...
              if ((t.hp === undefined || t.hp > 0) && dist < t.r) {
                  if (t.id === 'shield') {
                      const ang = Math.atan2(b.y - t.p.y, b.x - t.p.x);
                      const isSide = isMy ? (ang < -0.8 && ang > -Math.PI + 0.8) : (ang > 0.8 && ang < Math.PI - 0.8);
                      if (!isSide) continue;
                  }
      ...
      grenades.current.forEach((g, i) => {
          if (g.stage === 'moving') {
              ...
              const rd = 90 * (1.5 - g.life);
              ctx.strokeStyle = `rgba(255,255,255,${g.life})`; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(g.x, g.y, rd, 0, Math.PI*2); ctx.stroke();

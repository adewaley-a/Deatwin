...
export default function GamePage() {
  ...
  // Sparks is now actively used in the render loop to avoid ESLint errors
  const sparks = useRef([]); 
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const drawBox = (p, color, hp) => {
        if (hp <= 0) return;
        drawHPBar(p.x, p.y, hp, 200, color, 32);
        ctx.save();
      ...
      const drawS = (x, y, r, c, isE) => {
        ...
        if (!isE) {
            ...
            if (isCharging) {
                ...
                ctx.arc(0, 0, 45, 0, Math.PI * 2 * (Date.now() % 1000 / 1000)); ctx.stroke();
            }
        }
      ...
      // Sparks (Handling rendering to clear ESLint warning)

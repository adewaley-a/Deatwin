...
export default function GamePage() {
  ...
  // Bullet Firing (Vertex Tip Logic)
  useEffect(() => {
  ...
  const handleTouch = (e) => {
    ...
    for (let t of e.changedTouches) {
      ...
      if (e.type === "touchstart") {
        ...
        // Double Tap & Hold Logic for Grenade
        if (distToShooter < 45) {
          const now = Date.now();
          if (now - lastTap.current < 300 && grenades[role] > 0) {
            ...
            chargeTimer.current = setInterval(() => {
              prog += 0.05; // 2 seconds (100ms * 20 steps)
      ...
      if (e.type === "touchmove") {
        ...
        if (!type) continue; // FIX: No element "picked up", so no movement.
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Bullets Logic... [As before]
      
      frame = requestAnimationFrame(render);
    };
    render(); return () => cancelAnimationFrame(frame);
  }, [role, isCharging, chargeProgress]);

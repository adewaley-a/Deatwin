...
export default function GamePage() {
  ...
  const activeTouchType = useRef(null); 
  ...
  useEffect(() => {
    ...
    const drawShooter = (x, y, rot, color, isEnemy) => {
        ...
        if (!isEnemy) {
          ...
          ctx.beginPath(); ctx.moveTo(-40, 85); ctx.lineTo(40, 85); ctx.stroke();
          ctx.fillStyle = color; ctx.beginPath(); ctx.arc((rot / 1.22) * 40, 85, 10, 0, Math.PI * 2); ctx.fill();
        }
        ctx.rotate(rot); 
      ...
      [myBullets, enemyBullets].forEach((ref) => {
        ref.current.forEach((b, i) => {
          ...
          if (Math.hypot(b.x - target.x, b.y - target.y) < 25) {
            ...
            if (ref === enemyBullets) socket.current.emit("take_damage", { roomId, victimRole: role });
          }
    ...
    <div className="game-container" onTouchStart={handleTouch} onTouchMove={handleTouch} onTouchEnd={handleTouch}>
      <div className="header-dashboard">
        ...
        <div className="stat-box">
          ...
          <div className="mini-hp"><div className="fill local" style={{width: `${(myHP/400)*100}%`}}/></div>
          <span className="hp-val blue-text">{myHP} HP</span>
        </div>
      </div>
